...
function CheckOutContent() {
  ...
  const dropLat = params.get("dropUpLat");
  const dropLon = params.get("dropUpLon");
  ...
  const [paymentMethod, setPaymnetMethod] = useState<"cash" | "online" | null>(
    null
  );
  ...
  const handleCancelBooking = async () => {
    try {
      ...
      const { data } = await axios.get(
        `/api/booking/${currentBooking._id}/cancel`
      );
      setStatus("idle");
      console.log(data);
      setLoading(false);
      ...
      if (paymentMethod == "online") {
        ...
        const options = {
          ...
          handler: async function (response: any) {
            ...
            if (data.success) {
              setStatus("confirmed");
              window.location.href = `/user/ride/${currentBooking._id}`;
            }
          },
        };
        const paymnetObject = new (window as any).Razorpay(options);
        paymnetObject.open();
      } else {
        const { data } = await axios.get(
          `/api/booking/${currentBooking._id}/confirm`
        );
        setLoading(false);
        if (data.success) {
          setStatus("confirmed");
  ...
  return (
    <div className="min-h-screen bg-zinc-100 px-4 py-12">
      <div className="realtive max-w-6xl mx-auto z-10">
        <motion.div
              ...
              <div className="bg-zinc-50 border border-zinc-100 rounded-2xl overflow-hidden mb-8">
                <div className="flex gap-4 px-5 py-4 border-b border-zinc-100">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[9px] font-black uppercase tracking-[0.18em] text-zinc-400 mb-0.5">
                      Pickup
                    </p>
                      ...
                      {[
                        ...
                        return (
                          <motion.div
                            key={i}
                            whileTap={{ scale: 0.97 }}
                            onClick={() => setPaymnetMethod(p.id as any)}
                ...
                {status == "confirmed" && (
                  ...
                  >
                    ...
                    <motion.button
                      ...
                      onClick={() => {
                        window.location.href = `/ride/${currentBooking._id}`;
                      }}
