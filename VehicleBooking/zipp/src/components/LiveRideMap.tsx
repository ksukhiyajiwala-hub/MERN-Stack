...
function LiveRideMap({
  ...
  const [routeToPickUp, setRouterToPickUp] = useState<[number, number][]>([]);
  const [routeToDrop, setRouterToDrop] = useState<[number, number][]>([]);
  ...
  useEffect(() => {
    ...
    const getRoute = async (
      ...
      const { data } = await axios.get(
        `https://router.project-osrm.org/route/v1/bike/${startLon},${startLat};${endLon},${endLat}?overview=full&geometries=geojson`
      );
