...
function DocPreview({ label, url }: any) {
  ...
  return (
    <div className="bg-gray-50 rounded-2xl border overflow-auto shadow-sm">
      ...
      <div className="h-64 flex items-center justify-center bg-white">
        ...
        {isImage && <img src={url} className="w-full h-full object-fill " />}
        {isPdf && <iframe src={url} className="w-full h-full object-cover" />}
      </div>
      {url && (
        <a
          href={url}
