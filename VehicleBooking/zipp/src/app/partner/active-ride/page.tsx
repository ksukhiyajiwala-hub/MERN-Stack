...
function Page() {
  ...
  const status = (booking?.bookingStatus ?? "confirmed") as BookingStatus;
  ...
  useEffect(() => {
    async function fetch() {
      setLoading(true);
      try {
        const { data } = await axios.get("/api/partner/bookings/my-active");
        setBooking(data);
        if (!data) {
          setLoading(false);
          setBooking(null);
  ...
  if (booking == null) {
    return (
      <div className="bg-black w-full h-screen justify-center flex items-center text-[20px] text-white">
