...
function Pricing() {
  ...
  const handleStripePayment = async (planKey) => {
    if (!userData) {
      navigate("/");
      return;
    }
    ...
    try {
      ...
      setRedirectUrl(result.data.sessionUrl);
    } catch (error) {
      console.log(error);
      setLoading(null);
    }
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#050505] text-white px-6 pt-16 pb-24">
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute bottom-0 right-0 w-[400px] h-[400px] bg-indigo-600/20 rounded-full blur-[120px]" />
      </div>
      <button
      ...
      >
        ...
        <p className="text-zinc-400 text-lg">
          Buy creadits once. build anytime
        </p>
      </Motion.div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
        {plans.map((p, i) => (
          ...
          >
            ...
            <Motion.button
              disabled={loading}
              onClick={() => handleStripePayment(p.key)}
              whileTap={{ scale: 0.96 }}
